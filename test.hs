...
main = do
    ...
    code <- rawSystem "curl" [ "http://www.example.com/"
                             , "-v"
                             , "-L"
                             , "--socks5-hostname"
                             , "127.0.0.1:7777"
  ...
  where
    ...
    table1 = getTable "foobar!"
    decryptTable1 = toDecrypt table1
    table2 = getTable "barfoo!"
    decryptTable2 = toDecrypt table2
    tests = TestList [ table1 ~?= head target1
                     , decryptTable1 ~?= last target1
                     , table2  ~?= head target2
                     , decryptTable2 ~?= last target2
                     ]

    readOut h wait = do
        line <- S.hGetLine h
        let (s, _) = S.breakSubstring "starting" line
        if S.null s then putMVar wait ()
                    else readOut h wait
